...
msg-menu-connect =
    ...
    • пошаговая инструкция с настройкой приложения;
    • или сразу ссылка на подписку, если вы уже знаете, что с ней делать.

msg-menu-connect-link =
    🔗 Ваша ссылка на подписку:

    <code>{ $url }</code>

    Вставьте её в приложение VPN (например, Hiddify или v2rayNG), чтобы подключиться.
