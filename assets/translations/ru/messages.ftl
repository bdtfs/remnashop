...
msg-menu-tg-proxy =
    ...
    Бесплатный прокси для Telegram доступен в рамках вашего тарифа. Нажмите на кнопку ниже — прокси подключится автоматически.

msg-calls-beta =
    <b>📞 Звонки (бета)</b>

    Используйте этот профиль, если голосовые и видеозвонки прерываются в обычном VPN.

    1. Установите приложение <b>AmneziaWG</b> (или <b>Hiddify</b>).
    2. Импортируйте файл <code>compono-calls.conf</code> или отсканируйте QR-код ниже.
    3. Для Hysteria2 отсканируйте второй QR-код или скопируйте ссылку <code>hysteria2://</code>.
...
msg-menu-invite =
    ...
    Делитесь вашей уникальной ссылкой и получайте бонусы:
    • другу — <b>{ $invitee_discount }% скидка</b> на первую оплату;
    • вам — <b>{ $long_reward_amount } дней VPN</b>, если друг оплатит 3 месяца или год;
    • вам — обычная награда за короткую оплату.

    <i>Своя подписка не нужна: если её нет, мы просто выдадим вам новую на срок награды.</i>
...
msg-menu-invite-about =
    ...
    *[OTHER] { $accrual_strategy }
    ...
    <blockquote>
    Скидка <b>{ $invitee_discount }%</b> на первую оплату по вашей ссылке.
    </blockquote>

    <b>💎 Что вы получаете:</b>
    <blockquote>
    Награда приходит, даже если у вас нет своей подписки — в этом случае мы выдаём новую.
    ...
    *[1]
    ...
    Если друг оплатит тариф на <b>{ $long_reward_min_days }+ дней</b>, вы получите <b>{ $long_reward_amount } дней</b> VPN.
...
msg-promocode-configurator =
    ...
    • <b>Макс. дней</b>: { $purchase_discount_max_days }
...
msg-promocode-code-input = <b>🏷️ Введите код промокода</b>

    Отправьте текст промокода (латинские буквы и цифры).

msg-promocode-type-select = <b>🔖 Выберите тип награды</b>

msg-promocode-availability-select = <b>✴️ Выберите доступность</b>

msg-promocode-reward-input = <b>🎁 Введите значение награды</b>

    Для скидки: число от 1 до 100 (процент).

msg-promocode-lifetime-input = <b>⌛ Введите срок действия</b>

    Число дней (-1 = без ограничения).

msg-promocodes-list = <b>📃 Список промокодов</b>

    Выберите промокод для редактирования.

msg-promocode-search-input = <b>🔍 Поиск промокода</b>

    Введите код промокода для поиска.

msg-promocode-max-activations-input = <b>🔢 Введите лимит активаций</b>

    Число активаций (-1 = без ограничения).

msg-promocode-max-days-input = <b>📅 Введите макс. длительность для скидки</b>

    Максимальное кол-во дней подписки, на которое распространяется скидка (-1 = без ограничения).

msg-promocode-allowed-input = <b>👥 Введите разрешенных пользователей</b>

    Введите Telegram ID через запятую.

msg-promocode-plan-select = <b>📦 Выберите план для промокода</b>

msg-promocode-plan-duration = <b>⏳ Выберите длительность</b>

msg-subscription-promocode = <b>🎟 Активация промокода</b>

    Введите промокод для активации.

msg-subscription-promocode-success =
    <b>✅ Промокод <code>{ $code }</code> успешно активирован!</b>

    { $has_subscription ->
    [0] Скидка применена и будет учтена при следующей покупке.
    *[1] Чтобы начать пользоваться нашим сервисом, нажмите кнопку <code>`{ btn-subscription-connect }`</code> и следуйте инструкциям!
    }
